...
function compareArrays(arr1, arr2) {
  ...
  console.assert(
    ...
    "Assertion failed: Масиви не однакові!"
  );
...
function buildMatrix(size) {
  ...
  for (let i = 0; i < size; i++) {
    ...
    const row = []; // Створюємо порожній масив, який буде рядком матриці.
    for (let j = 0; j < size; j++) {
      // Цикл всередені циклу для заповнення колонок матриці, лічильник j має бути від 0 до розміру матриці.
      const rumdomNumber = Math.floor(Math.random() * 10); // Генеруємо випадкове число від 0 до 9 округляємо його до меншого цілого,
      row.push(rumdomNumber); // і додаємо його до рядка.
    }
    matrix.push(row); // Додаємо рядок до матриці.
  }
...
function displayGroupedInfo(students) {
  for (let student of students) {
    ...
    console.log(`Вік: ${student.age}`); // Виведемо корисне повідомлення з текстом `Вік: ${student.age}`
    console.log(`Предмет: ${student.subject}`); // Виведемо корисне повідомлення з текстом `Предмет: ${student.subject}`
...
function countVowelsAndConsonants(word) {
  const vowels = "aeiou"; // Створюємо рядок де будуть всі голосні "aeiou"
  let vowelsCount = 0;
  let consonantsCount = 0;
  for (let ltr of word) {
    // Перебираємо кожну літеру у слові за допомогою for of
    const lowercase = ltr.toLowerCase();
  ...
  console.log("Голосних:", vowelsCount);
  console.log("Приголосних:", consonantsCount);
  vowelsCount = 0;
  consonantsCount = 0;
  // Скидаємо лічильники
}
